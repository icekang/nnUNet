...
DATASET_ID=308
TRAINER=nnUNetTrainerScaleAnalysis40
...
cpu_VISIBLE_DEVICES=1 nnUNetv2_train $DATASET_ID $CONFIG 1 -tr $TRAINER -device cuda &  # Task 2 (Fold 1) runs on GPU 1
...
CUDA_VISIBLE_DEVICES=0 nnUNetv2_train $DATASET_ID $CONFIG 2 -tr $TRAINER -device cuda & # Task 3 (Fold 2) runs on GPU 0
