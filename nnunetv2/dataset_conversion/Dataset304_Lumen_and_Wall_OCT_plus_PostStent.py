...
def get_train_splits():
    ...
    train_post_stent =['STENT_0000', 'STENT_0001', 'STENT_0002', 'STENT_0003', 'STENT_0004']
    return train, train_post_stent
...
if __name__ == '__main__':
    ...
    for case_id in train_post_stent:
        shutil.copy(join(lawoct_post_stent_dir, 'images', f'{case_id}_0000.nii.gz'), join(imagestr, f'{case_id}_0000.nii.gz'))
        copy_LaWOCT_segmentation_and_convert_labels_to_nnUNet(
            join(lawoct_post_stent_dir, 'labels', f'{case_id}.nii.gz'),
            join(labelstr, f'{case_id}.nii.gz')
        )
    for case_id in test:
    ...
    generate_dataset_json(out_base,
                          ...
                          num_training_cases=len(train) + len(train_post_stent),
                          file_ending='.nii.gz',
